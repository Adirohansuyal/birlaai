...
from utils.ui_helpers import (
    local_css, display_header, display_medical_disclaimer, 
    display_risk_badge, display_medical_attention_alert,
            ...
            with st.form("user_info_form"):
                    ...
                    with system_tabs[i]:
                        ...
                        for symptom in system_symptoms:
                            if symptom not in st.session_state.all_selected_symptoms:
                                st.session_state.all_selected_symptoms.append(symptom)
                ...
                if st.session_state.all_selected_symptoms:
                    ...
                    for symptom in st.session_state.all_selected_symptoms:
                        st.markdown(f"<div style='padding: 0.3rem 0.8rem; background-color: #E3F2FD; border-radius: 20px; margin-bottom: 0.5rem; display: inline-block; margin-right: 0.5rem;'>{symptom}</div>", unsafe_allow_html=True)
...
def display_results(analysis, symptoms):
    ...
    with st.container():
        ...
        with col2:
            ...
            st.markdown("<h4>Your Reported Symptoms:</h4>", unsafe_allow_html=True)
        ...
        if not possible_conditions:
            st.info("No specific conditions identified based on your symptoms. Please consult a healthcare professional for a proper diagnosis.")
        else:
        ...
        if sources:
            ...
            for source in sources:
                st.markdown(f'<p style="margin-bottom: 0.5rem;"><a href="{source}" target="_blank">{source}</a></p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        else:
        ...
        if USING_AI:
            ...
            if len(symptoms) > 0:
                ...
                display_chat_message(f"I have these symptoms: {', '.join(symptoms)}", is_user=True)
                ...
                follow_up = st.text_input("Ask a follow-up question:", placeholder="Type your health question here...")
                st.markdown('</div>', unsafe_allow_html=True)
